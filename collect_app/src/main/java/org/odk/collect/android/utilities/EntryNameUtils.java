...
public final class EntryNameUtils {
    ...
    private EntryNameUtils() {
    }

    public static String normalizeEntryName(String entryName, boolean returnNullIfNothingChanged) {
        if (entryName == null) {
            return null;
        }

        Matcher matcher = CONTROL_CHAR_PATTERN.matcher(entryName);
        return matcher.find()
                ? matcher.replaceAll(" ")
                : (returnNullIfNothingChanged ? null : entryName);
    }

    // Create a sanitized filename prefix from the given form name. No extension is added.
    public static String formatFilenameFromEntryName(String entryName) {
        if (entryName == null) {
            return null;
        }
        // Keep alphanumerics, replace others with a space
        String fileName = entryName.replaceAll("[^\\p{L}\\p{Digit}]", " ");
